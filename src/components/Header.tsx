...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header id="main-header" className="bg-white border-b border-slate-200 sticky top-0 z-20">
      ...
      <div className="px-3 py-2.5 sm:px-6 sm:py-3 flex flex-col md:flex-row md:items-center justify-between gap-2.5 sm:gap-4">
        {/* Mobile Top Row: Hamburger + Brand + Version + Date */}
        <div className="flex items-center justify-between w-full md:w-auto gap-2">
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="md:hidden flex items-center justify-center p-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 min-h-[44px] min-w-[44px] transition-colors"
            >
              <Menu className="w-5 h-5" />
            </button>

            {/* Logo/Título visível no celular quando a sidebar está oculta */}
            <div className="flex items-center gap-2 md:hidden">
              <div className="w-8 h-8 rounded-lg bg-emerald-600 flex items-center justify-center text-white shadow-xs">
                <Clock className="w-4 h-4" />
              </div>
              <div className="leading-tight">
                <span className="font-bold text-xs text-slate-800 tracking-tight block">GESTÃO DE PLANTÕES</span>
                <span className="text-[10px] text-slate-400">Escala Hospitalar</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 md:hidden">
            {/* Botão de Instalação PWA Mobile */}
            <button
              id="btn-pwa-install-mobile"
              onClick={onOpenPWAInstall}
              title="Instalar App no celular (PWA)"
              className="flex items-center justify-center w-8 h-8 rounded-md bg-emerald-600 hover:bg-emerald-700 text-white shadow-xs cursor-pointer transition-colors"
            >
              <Smartphone className="w-4 h-4" />
            </button>

            {/* Indicador de Nuvem Mobile (Clicável para ver opções de publicação web) */}
            <button
              id="badge-cloud-mobile"
              onClick={onOpenPublicacao}
              title="Acesso Web Ativo: Clique para ver opções de publicação em outro endereço"
              className="flex items-center justify-center w-8 h-8 rounded-md bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-300 cursor-pointer transition-colors"
            >
              <Globe className="w-3.5 h-3.5 text-emerald-600 animate-pulse" />
            </button>

            {/* Contador / Indicador de Versão Mobile */}
            <button
              id="btn-header-version-info-mobile"
              onClick={onOpenVersionModal}
              title="Clique para ver informações e contador de versão"
              className="flex items-center gap-1 px-2 py-1 bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-300 rounded-md text-[11px] font-mono font-bold transition-colors cursor-pointer min-h-[36px]"
            >
              <span>{versionInfo?.version || 'V12.0'}</span>
            </button>

            {/* Data Base no Mobile */}
            <div className="flex items-center gap-1 bg-slate-100 px-2 py-1 rounded-lg border border-slate-200 min-h-[36px]">
              <Calendar className="w-3.5 h-3.5 text-slate-600 shrink-0" />
              <input
                id="input-header-selected-date-mobile"
                type="date"
                value={selectedDate}
                onChange={(e) => onDateChange(e.target.value)}
                className="text-[11px] font-medium text-slate-900 bg-transparent border-none focus:outline-none cursor-pointer w-28"
              />
            </div>
          </div>
        </div>

        {/* Global Search (Apenas ADM tem busca geral abrangente com matrícula/usuários) */}
        <div className="relative w-full md:flex-1 md:max-w-md">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-8 py-2 md:py-1.5 bg-slate-50 hover:bg-slate-100/80 focus:bg-white border border-slate-200 rounded-lg text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all min-h-[38px] md:min-h-0"
            />
            {searchTerm && (
              <button 
              ...
              {matchedProfissionais.length === 0 && matchedSetores.length === 0 ? (
                ...
                <div className="divide-y divide-slate-100">
                  {matchedProfissionais.map(p => {
                    ...
                    return (
                      <div 
                        ...
                        className="p-2.5 hover:bg-slate-50 rounded cursor-pointer flex items-center justify-between"
                      >
                        <div className="flex items-center gap-2">
                  ...
                  {matchedSetores.map(s => (
                    <div key={s.id} className="p-2.5 hover:bg-slate-50 rounded flex items-center justify-between">
                      <div className="flex items-center gap-2">
        ...
        {/* Right Desktop Controls (Escondido no mobile para evitar poluição visual) */}
        <div className="hidden md:flex items-center gap-3">
          {/* Active Work Date Desktop */}
          ...
          {isAdmin && (
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 rounded-lg transition-colors cursor-pointer"
            >
          ...
          <button
            ...
            onClick={onOpenPWAInstall}
            title="Instalar este sistema como aplicativo (PWA) no seu computador ou celular"
            className="flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg shadow-xs transition-transform active:scale-95 cursor-pointer"
          >
            <Smartphone className="w-3.5 h-3.5" />
            <span className="hidden lg:inline">Instalar App (PWA)</span>
            <span className="lg:hidden">App PWA</span>
          </button>

          {/* Indicador de Sincronização Web em Nuvem (Clicável para ver opções de publicação web) */}
          <button 
            id="badge-cloud-web-sync"
            onClick={onOpenPublicacao}
            className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 bg-emerald-50 hover:bg-emerald-100 border border-emerald-300 rounded-md text-[11px] text-emerald-800 font-medium cursor-pointer transition-colors"
            title="Acesso Web Ativo: Clique para ver opções de publicação em outro endereço ou baixar pacote web."
          >
            <Globe className="w-3.5 h-3.5 text-emerald-600 animate-pulse shrink-0" />
            <span className="hidden lg:inline">Web Nuvem Ativa</span>
            <span className="lg:hidden">Web Nuvem</span>
          </button>

          {/* Contador / Indicador de Versão Desktop */}
          <button
            id="btn-header-version-info"
            onClick={onOpenVersionModal}
            title="Clique para ver informações e contador de versão"
            className="flex items-center gap-1 px-2.5 py-1 bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-300/80 rounded-md text-[11px] font-mono font-bold transition-colors cursor-pointer"
          >
            <span>{versionInfo?.version || 'V12.0'}</span>
          </button>
        </div>
